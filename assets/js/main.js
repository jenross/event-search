...
<<<<<<< HEAD
    ...
    for (let i = 0; i < events; i++) {
      ...
      let location = $(
        `<td class='event-data local' id='${
          response.events[i].venue.postal_code
        }'>${response.events[i].venue.display_location}</td>`
      );
      let venue = $(
        `<td class='event-data'> ${response.events[i].venue.name} </td>`
      );
      let moreInfo = $(
        `<td><a class="btn btn-primary" id='resultsBtn' type="button" href='#' role="button" id=customButtons id_zip='${
          response.events[i].venue.postal_code
        }' id_location='${
          response.events[i].venue.display_location
        }'>More info</a></td>`
      );

      let results = $("<ul>").addClass(
        "each-event d-flex flex-row justify-content-around"
      );
...
$(document).on("click", "#weatherBtn", function() {
  ...
  $.ajax({
    ...
    let local =
      ...
      " " +
      response[0].AdministrativeArea.LocalizedName;
    ...
    $.ajax({
      ...
      results.forEach(element => {
        ...
        let sun =
          moment(element.Sun.Rise, "YYYY-MM-DDThh:mm:ss").format("h:mm") +
          " AM/" +
          moment(element.Sun.Set, "YYYY-MM-DDThh:mm:ss").format("h:mm") +
          " PM";
